...
# Create global ask_human tool instance
from app.tool.ask_human import AskHuman
ask_human_tool = AskHuman()
...
def submit_response():
    """Submit response to ask_human tool"""
    try:
        data = request.get_json()
        if not data or 'question_id' not in data or 'response' not in data:
            return jsonify({'error': 'Invalid request. "question_id" and "response" are required.'}), 400
        ...
        if ask_human_tool.submit_response(question_id, response_text):
            return jsonify({'success': True, 'message': 'Response submitted successfully'})
        else:
            return jsonify({'error': 'No pending question found for the given ID'}), 400

    except Exception as e:
        logger.error(f"Error submitting response: {e}")
        return jsonify({'error': str(e)}), 500

@app.route('/api/pending-question', methods=['GET'])
def get_pending_question():
    """Get the current pending question from the ask_human tool"""
    try:
        pending_question = ask_human_tool.get_pending_question()
        if pending_question:
            question_id, question_text = pending_question
            return jsonify({'question_id': question_id, 'question': question_text})
        else:
            return jsonify({'question_id': None, 'question': None})
    except Exception as e:
        logger.error(f"Error getting pending question: {e}")
        return jsonify({'error': str(e)}), 500
...
def submit_response():
    ...
    if not question_id or not response:
        return jsonify({'status': 'error', 'message': 'Missing question_id or response'}), 400

    # Find the correct ask_human_tool instance
    # This assumes a single, global instance.
    # For multi-user setups, this would need to be handled differently.
